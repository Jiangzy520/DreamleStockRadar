...
WATCHLIST_TXT = ALLTICK_MANAGER_DIR / "watchlist.txt"
API_TXT = ALLTICK_MANAGER_DIR / "apis.txt"
ASSIGNMENT_CSV = ALLTICK_MANAGER_DIR / "stock_assignments.csv"
API_ASSIGNMENT_CSV = ALLTICK_MANAGER_DIR / "api_assignments.csv"
ASSIGNMENT_SETTINGS_JSON = ALLTICK_MANAGER_DIR / "settings.json"
...
def _load_assignment_settings(path: Path = ASSIGNMENT_SETTINGS_JSON) -> dict[str, Any]:
    settings = DEFAULT_ASSIGNMENT_SETTINGS.copy()
    try:
    ...
    try:
        ...
        settings["max_stocks_per_api"] = DEFAULT_ASSIGNMENT_SETTINGS["max_stocks_per_api"]
    if settings.get("distribution_mode") not in {"balanced", "sequential"}:
        settings["distribution_mode"] = DEFAULT_ASSIGNMENT_SETTINGS["distribution_mode"]
    return settings


def _build_assignment_exports(
    rows: list[dict[str, str]],
    tokens: list[str],
    settings: dict[str, Any],
) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    max_per_api = max(int(settings.get("max_stocks_per_api", 2) or 2), 1)
    distribution_mode = str(settings.get("distribution_mode") or "balanced")
    api_rows: list[dict[str, Any]] = []
    stock_rows: list[dict[str, Any]] = []

    if not tokens:
        for stock_seq, row in enumerate(rows, start=1):
            stock_rows.append(
                {
                    "stock_seq": stock_seq,
                    "code": row["code"],
        ...
        return api_rows, stock_rows, {
            ...
            "max_stocks_per_api": max_per_api,
        }

    buckets: list[list[dict[str, str]]] = [[] for _ in tokens]
    unassigned_rows: list[dict[str, str]] = []

    if distribution_mode == "sequential":
        bucket_index = 0
        for row in rows:
            while bucket_index < len(tokens) and len(buckets[bucket_index]) >= max_per_api:
                bucket_index += 1
            if bucket_index >= len(tokens):
        ...
        for row in rows:
            ...
            for _ in range(len(tokens)):
                bucket_index = cursor % len(tokens)
                cursor += 1
    ...
    stock_seq = 1
    for api_seq, (api, bucket) in enumerate(zip(tokens, buckets), start=1):
        api_rows.append(
            {
                "api_seq": api_seq,
                "api": api,
                "stock_count": len(bucket),
                "stocks": " | ".join(f"{item['symbol']} {item.get('name') or item['code']}" for item in bucket),
                "status": "used" if bucket else "idle",
            }
        )
        for slot, row in enumerate(bucket, start=1):
            stock_rows.append(
                {
                    "stock_seq": stock_seq,
                    "code": row["code"],
    ...
    used_api_count = sum(1 for item in api_rows if int(item["stock_count"]) > 0)
    return api_rows, stock_rows, {
        "assigned_count": len(rows) - len(unassigned_rows),
        "unassigned_count": len(unassigned_rows),
        "used_api_count": used_api_count,
        "unused_api_count": max(len(tokens) - used_api_count, 0),
        "api_total_count": len(tokens),
        "distribution_mode": distribution_mode,
        "max_stocks_per_api": max_per_api,
    }


def _write_assignment_rows(
    rows: list[dict[str, str]],
    tokens: list[str],
    path: Path = ASSIGNMENT_CSV,
    api_path: Path = API_ASSIGNMENT_CSV,
) -> dict[str, Any]:
    settings = _load_assignment_settings()
    api_rows, stock_rows, summary = _build_assignment_exports(rows, tokens, settings)
    ...
    with api_path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["api_seq", "api", "stock_count", "stocks", "status"])
        writer.writeheader()
...
def _write_daily_watchlist(rows: list[dict[str, str]], target_day: datetime) -> dict[str, str]:
    ...
    api_assignment_path = WATCHLIST_DAILY_DIR / f"api_assignments_{day_token}.csv"
    _write_watchlist_rows(rows, csv_path)
    txt_path.write_text(_watchlist_codes_text(rows) + ("\n" if rows else ""), encoding="utf-8")
    assignment_meta = _write_assignment_rows(rows, _load_api_tokens(), assignment_path, api_assignment_path)
    return {
        "csv_name": csv_path.name,
        "txt_name": txt_path.name,
        "assignment_name": assignment_path.name,
        "api_assignment_name": api_assignment_path.name,
...
def _watchlist_editor_snapshot() -> dict[str, Any]:
    ...
    settings = _load_assignment_settings()
    api_total_count = len(_load_api_tokens())
    assigned_count, used_api_count = _assignment_stats(ASSIGNMENT_CSV)
    return {
        ...
        "unassigned_count": max(len(rows) - assigned_count, 0),
        "api_total_count": api_total_count,
        "used_api_count": used_api_count,
        "unused_api_count": max(api_total_count - used_api_count, 0),
...
def _write_watchlist_text_export(rows: list[dict[str, str]], path: Path = WATCHLIST_TXT) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
...
def create_app(auto_connect: bool = True) -> Flask:
    ...
    def api_realtime_watchlist_save() -> Any:
        try:
            ...
            _write_watchlist_text_export(rows, WATCHLIST_TXT)
            assignment_meta = _write_assignment_rows(rows, _load_api_tokens(), ASSIGNMENT_CSV, API_ASSIGNMENT_CSV)
            ...
            snapshot.update(
                {
                    ...
                    "daily_api_assignment_csv": daily_meta["api_assignment_name"],
                    "assigned_count": assignment_meta["assigned_count"],
                    "unassigned_count": assignment_meta["unassigned_count"],
                    "used_api_count": assignment_meta["used_api_count"],
                    "unused_api_count": assignment_meta["unused_api_count"],
                    "api_total_count": assignment_meta["api_total_count"],
                    "distribution_mode": assignment_meta["distribution_mode"],
                    "max_stocks_per_api": assignment_meta["max_stocks_per_api"],
            ...
            message_parts = [
                ...
                (
                    ...
                    f"已用 API {assignment_meta['used_api_count']}/{assignment_meta['api_total_count']}，"
                    f"单个 API 最多 {assignment_meta['max_stocks_per_api']} 只"
                ),
                reload_meta["message"],
            ]
            return jsonify(
